...
const ignoredFiles = ['docs/404.mdx']
...
async function generateEmbeddings() {
  ...
  if (!shouldRefresh) {
    console.log('Checking which docs are new or have changed')
  } else {
  ...
  for (const embeddingSource of embeddingSources) {
    ...
    try {
      ...
      if (!shouldRefresh && existingPage?.checksum === checksum) { //@ts-ignore
      ...
      if (existingPage) {
        ...
        const { error: deletedocsectionError } = await supabaseClient
          .from('nods_page_section')
          .delete()
          .filter('page_id', 'eq', existingPage.id)

        if (deletedocsectionError) {
          throw deletedocsectionError
        }
      }
      ...
      for (const { slug, heading, content } of sections) {
        ...
        try {
          ...
          const { error: insertdocsectionError, data: docsection } = await supabaseClient
            .from('nods_page_section')
            .insert({
              page_id: page.id,
              slug,
          ...
          if (insertdocsectionError) {
            throw insertdocsectionError
          }
        } catch (err) {
